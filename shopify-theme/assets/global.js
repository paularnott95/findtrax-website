...
(function () {
  function insertAdviceSupportBox() {
    var article = document.querySelector('.seo-advice-article');
    if (!article || article.querySelector('.seo-advice-support-box, .support-missing-alerts-box')) return;

    var body = article.querySelector('.seo-advice-article__body');
    if (!body) return;

    var supportBox = document.createElement('section');
    supportBox.className = 'seo-advice-support-box';
    supportBox.setAttribute('data-advice-support-block', 'true');
    supportBox.innerHTML = [
      '<!-- MA_SUPPORT_BOX_REFRESH_FINAL -->',
      '<div>',
      '<p class="seo-advice-support-box__eyebrow">SUPPORT MISSING ALERTS</p>',
      '<h2>Help Keep Missing Alerts Running 24/7</h2>',
      '<p>Support the team working behind every alert, helping missing people information stay visible, updated, and accessible worldwide.</p>',
      '<p>Safe • Trusted • Public awareness platform</p>',
      '<ul><li>Real-time alert visibility</li><li>Small team, global impact</li><li>Every contribution helps reach more people</li></ul>',
      '</div>',
      '<div class="seo-advice-support-box__actions">',
      '<a href="https://buymeacoffee.com/missingalerts" class="seo-advice-button" target="_blank" rel="noopener noreferrer">Support the Platform<br><small>Help keep alerts live →</small></a>',
      '<p>Built independently • Supported by the community</p>',
      '</div>',
    ].join('');

    body.insertAdjacentElement('afterend', supportBox);
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', insertAdviceSupportBox);
  } else {
    insertAdviceSupportBox();
  }
})();
