...
export function parseHtmlCase(html, source = {}) {
  ...
  const imageUrl = absoluteUrl(meta(html, 'og:image') || firstContentImage(html), source.sourceUrl || source.url)
  return {
    title,
    name: source.name || '',
    summary: description,
...
function parsePoliceScotlandCase(html, source = {}) {
  ...
  const imageUrl = absoluteUrl(firstNonLogoImage(html) || meta(html, 'og:image'), sourceUrl)
