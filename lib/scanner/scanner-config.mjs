...
const envFiles = ['.env', '.env.local', '.env.production', '.env.shopify']
...
export function scannerConfig() {
  ...
  return {
    ...
    shopifyAdminAccessToken: process.env.SHOPIFY_ADMIN_ACCESS_TOKEN || process.env.SHOPIFY_ADMIN_TOKEN || process.env.SHOPIFY_ACCESS_TOKEN || '',
    shopifyRefreshToken: process.env.SHOPIFY_REFRESH_TOKEN || process.env.SHOPIFY_OFFLINE_REFRESH_TOKEN || '',
    shopifyClientId: process.env.SHOPIFY_CLIENT_ID || process.env.SHOPIFY_APP_API_KEY || '',
    shopifyClientSecret: process.env.SHOPIFY_CLIENT_SECRET || process.env.SHOPIFY_API_SECRET || '',
    shopifyAccessTokenExpiresAt: process.env.SHOPIFY_ACCESS_TOKEN_EXPIRES_AT || '',
    shopifyTokenPersistence: process.env.SHOPIFY_TOKEN_PERSISTENCE || '',
...
export function credentialPresence() {
  ...
  return {
    ...
    SHOPIFY_REFRESH_TOKEN: Boolean(config.shopifyRefreshToken),
    SHOPIFY_CLIENT_ID: Boolean(config.shopifyClientId),
    SHOPIFY_CLIENT_SECRET: Boolean(config.shopifyClientSecret),
    SHOPIFY_ACCESS_TOKEN_EXPIRES_AT: Boolean(config.shopifyAccessTokenExpiresAt),
    SHOPIFY_TOKEN_PERSISTENCE: Boolean(config.shopifyTokenPersistence),
