export function dedupeCases(incoming, existing = []) {
  ...
  for (const item of incoming) {
    ...
    if (sourceUrl && seenUrls.has(sourceUrl)) {
      duplicates.push({ ...item, sourceUrl: item.sourceUrl, reason: 'duplicate-source-url', duplicate: true })
      continue
    }
    if (item.externalId && seenExternalIds.has(externalKey)) {
      duplicates.push({ ...item, sourceUrl: item.sourceUrl, reason: 'duplicate-external-id', duplicate: true })
      continue
    }
    if (item.duplicateKey && seenKeys.has(item.duplicateKey)) {
      duplicates.push({ ...item, sourceUrl: item.sourceUrl, reason: 'duplicate-name-date-country', duplicate: true })
      continue
    }
